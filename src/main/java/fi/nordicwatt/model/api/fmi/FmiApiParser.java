...
public class FmiApiParser implements APIParserInterface<WeatherModel> {
     ...
     */
    @Override
    public WeatherModel parseToDataObject(DataRequest request, String response)
        ...
        if (responseDataAsString.isEmpty()) {
            Logger.log(
                    "Data requested from FMI API was empty and contained no values. This means that the data is not available for the requested time period. Returning empty datamodel.",
                    "FmiApiParser.log");
            return new WeatherModel(request.getDataType(), request.getDataType().getUnit(),
                    request.getStarttime(), request.getLocation(), new Double[0]);
        }
        List<Double> responseData = extractDoubles(responseDataAsString);
        Double[] responseDataArray = new Double[responseData.size()];

        String location = getStringBetween(
                response, "<target:region codeSpace=" + "\""
                        + "http://xml.fmi.fi/namespace/location/region" + "\"" + ">",
                "</target:region>");
        CharSequence firstEntryTimestamp =
                getStringBetween(response, "<gml:beginPosition>", "</gml:beginPosition>");
        ...
        LocalDateTime dateTime = DateTimeConverter
                .gmtTimeToFinnishTime(LocalDateTime.parse(firstEntryTimestamp, originalFormat));
        ...
        WeatherModel model =
                new WeatherModel(dataType, dataType.getUnit(), dateTime, location, responseArray);

        return model;
    }

    /**
     * getStringBetween - Gets a string between two strings. Exclude the start and end strings from
     * the result.
     * 
    ...
    public static String getStringBetween(String originalStr, String startStr, String endStr) {
        ...
        if (originalStr.contains(startStr) && originalStr.contains(endStr)) {
            ...
            int endIndex = originalStr.indexOf(endStr, startIndex); // Searching for endStr after
                                                                    // the startIndex
